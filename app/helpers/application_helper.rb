module ApplicationHelper
  def get_twitter_card_info(post)
    ...
    if post.present?
      if post.id.present?
        twitter_card[:url] = "https://serifoo.herokuapp.com/posts/#{post.id}"
        twitter_card[:image] = "https://s3-ap-northeast-1.amazonaws.com/kakugen/images/#{post.id}.png"
      else
        twitter_card[:url] = 'https://serifoo.herokuapp.com/'
